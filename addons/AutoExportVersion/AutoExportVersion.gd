...
const VERSION_SCRIPT_PATH = "res://Tools/version.gd"
...
func _fetch_version(features: PackedStringArray, is_debug: bool, path: String, flags: int) -> String:
	...
	var output := []
	#OS.execute("git", PackedStringArray(["rev-list", "--count", "HEAD"]), true, output)
...
func _enter_tree() -> void:
	...
	add_tool_menu_item("Print Current Version", Callable(self, "print_version"))
	
	if not FileAccess.file_exists(VERSION_SCRIPT_PATH):
...
func print_version(ud):
	OS.alert("Current game version: %s" % _fetch_version(PackedStringArray(), true, "", 0))
