...
@onready var tabs := $VBoxContainer/HBoxContainer/TabBar
...
@onready var icon_generator := $SubViewport

@onready var can_draw := $VBoxContainer/CheckBox
@onready var rand_rot := $VBoxContainer/CheckBox2
@onready var min_sc := $VBoxContainer/HBoxContainer2/HSlider
@onready var max_sc := $VBoxContainer/HBoxContainer3/HSlider
...
func remove_tab_confirm() -> void:
	...
	scrolls.remove_at(tab_to_remove)
	refresh_tabs()
	...
	for i in ceil((tab_scenes.size() + 1) / 5.0) * 5:
		...
		if i < tab_scenes.size() and tab_scenes[i]:
			...
			if icon:
				...
				var instance = load(tab_scenes[i]).instantiate()
...
func remove_scene(slot: int):
	var tab_scenes = scenes[scenes.keys()[tabs.current_tab]]
	tab_scenes[slot] = ""
	while not tab_scenes.is_empty() and tab_scenes.back() == "":
		tab_scenes.pop_back()
...
func add_slot(scene_id: int) -> Control:
	...
	slot.connect("request_icon", Callable(self, "generate_icon").bind(slot))
	slot.connect("scene_set", Callable(self, "scene_set").bind(scene_id))
	slot.connect("remove_scene", Callable(self, "remove_scene").bind(scene_id))
	return slot
