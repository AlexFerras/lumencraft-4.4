...
func _enter_tree() -> void:
	if not get_meta("_edit_initialized_", false):
...
func update_label(force := false):
	...
	var grabber_size: Vector2 = slider.get_icon("Grabber").get_size()
	if vertical:
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if custom_slider_path.is_empty():
		if not get_parent() is Slider:
			warnings.append(SLIDER_WARNING)
	else:
		if not get_node(custom_slider_path) is Slider:
			warnings.append(SLIDER_WARNING2)
