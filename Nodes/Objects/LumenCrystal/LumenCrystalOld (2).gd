	...
	if not Engine.is_editor_hint():
		...
		if next_check <= 0.0:
			...
			if prev_scale and not ray_group.is_any_ray_solid():
				get_pixel_map().update_material_mask_rotated(prev_pos, $Sprite2D.texture.get_data(), Const.Materials.EMPTY, Vector3(prev_scale.x, prev_scale.y, prev_rot), 1 << Const.Materials.LUMEN)
...
func remove_pixels():
	...
	var image: Image = $Sprite2D.texture.get_data()
...
func can_draw() -> bool:
	...
	if not $Sprite2D.texture.get_data(): # W ogóle możliwe??
		return false
